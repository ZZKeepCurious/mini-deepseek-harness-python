...
def _notice_messages(server: AcpServer, session_id: str) -> list:
    """会话日志里 source.kind == 'model-selection' 的 user 消息。"""
    events = server.sessions[session_id]["session"].events
    return [
        ev["data"] for ev in events
        if ev["type"] == "user/message"
        and (ev["data"].get("source") or {}).get("kind") == "model-selection"
    ]


class TestModelSelectionNotice(unittest.TestCase):
    def setUp(self):
        self.server = AcpServer(adapter=_CatalogAdapter())
        self.session_id = self.server.new_session(_CWD)["sessionId"]
    ...
    def test_no_notice_on_first_prompt(self):
        self.server.prompt(self.session_id, [{"type": "text", "text": "hi"}])
        self.assertEqual(_notice_messages(self.server, self.session_id), [])

    def test_no_notice_without_switch(self):
        self.server.prompt(self.session_id, [{"type": "text", "text": "hi"}])
        self.server.prompt(self.session_id, [{"type": "text", "text": "again"}])
        self.assertEqual(_notice_messages(self.server, self.session_id), [])

    def test_notice_appended_after_model_switch(self):
        self.server.prompt(self.session_id, [{"type": "text", "text": "hi"}])
        self._set("model", _model_val("fake", "fake-model-pro"))
        self.server.prompt(self.session_id, [{"type": "text", "text": "again"}])
        notices = _notice_messages(self.server, self.session_id)
        self.assertEqual(len(notices), 1)
        source = notices[0]["source"]
        self.assertEqual(source["kind"], "model-selection")
        self.assertEqual(source["form"], "notice")
        self.assertEqual(source["summary"], "fake-model → fake-model-pro")
        self.assertEqual(len(notices[0]["content"]), 1)
        self.assertEqual(
            notices[0]["content"][0]["text"],
            "[model changed: assistant turns above this point were generated by "
            "fake-model; the session continues with fake-model-pro]")

    def test_cross_provider_route_label(self):
        self.server.prompt(self.session_id, [{"type": "text", "text": "hi"}])
        self._set("model", _model_val("other", "o-1"))
        self.server.prompt(self.session_id, [{"type": "text", "text": "again"}])
        notices = _notice_messages(self.server, self.session_id)
        self.assertEqual(len(notices), 1)
        self.assertEqual(notices[0]["source"]["summary"], "fake/fake-model → other/o-1")

    def test_effort_only_change_adds_no_notice(self):
        self.server.prompt(self.session_id, [{"type": "text", "text": "hi"}])
        self._set("reasoning_effort", "high")
        self.server.prompt(self.session_id, [{"type": "text", "text": "again"}])
        self.assertEqual(_notice_messages(self.server, self.session_id), [])
