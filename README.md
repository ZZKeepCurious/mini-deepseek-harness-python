...
| Filesystem domain (`ctx.fs` seam + local/sandbox backends + model-facing `read`/`write`/`edit`/`str_replace_editor`/`glob`/`grep`/`present` tools; observed-state mutation guards + sandbox fence) | `packages/fs/{fs,fs-local,fs-sandbox,fs-observation-policy,tool-fs,tool-str-replace-editor,tool-fs-search,tool-present}` |
...
| Output spill (`ctx.spillStore` seam + per-session private-dir local store + `tools/post-execute` policy: oversized all-text results spilled with head/tail preview and retrieval hint) | `packages/spill/{spill,spill-local,spill-policy}` |
| Workspaces (`ctx.workspaces` registry: realpath-canonical path identity, stable uuid, ordered session account with attach/insertBefore/detach, live directory status) | `packages/workspace/workspace` |
...
| Workspace control Remote (`ctx.workspaceController`, Remote namespace `workspace/*`: create/rename/delete/insertBefore/insertSessionBefore/archiveSession/unarchiveSession + `follow` baseline/upsert/remove/order/archived increments; registry order + archived-session set + `workspace/changed` notifications) | `packages/api/workspace-controller` |
| Workspace file preview Remote (`ctx.workspaceFiles`, Remote namespace `workspaceFiles/*`: bounded line `read`, base64 `readBytes`/`readAll`, `readRelated`, `stat`, workspace-scoped `list` + `changes` over `fs/observed`; `workspace-file/*` error set) | `packages/api/workspace-files` |
