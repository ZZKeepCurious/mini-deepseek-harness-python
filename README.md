...
**Mini DeepSeek Harness** is a production-oriented Python implementation of [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`) — the open-source agent harness developed by [DeepSeek AI](https://deepseek.com) — written by re-implementing the upstream core contracts bottom-up for clarity, and built for **production readiness** (**mature open-source libraries first**, stdlib only where no equivalent library fits — `httpx` for the DeepSeek SSE transport, optional `pyyaml`, and an optional `[web]` extra: `fastapi` + `uvicorn` for the HTTP/SSE transport layer).
