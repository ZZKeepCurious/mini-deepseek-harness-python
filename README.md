...
| Filesystem domain (`ctx.fs` seam + local/sandbox backends + model-facing `read`/`write`/`edit`/`str_replace_editor`/`glob`/`grep` tools; observed-state mutation guards + sandbox fence + watchdog-backed `watch`) | `packages/fs/{fs,fs-local,fs-sandbox,fs-observation-policy,tool-fs,tool-str-replace-editor,tool-fs-search}` |
...
| Session checkpoint policy (semantic durability barriers before a model request, a top-level tool body, and each step boundary; fail-closed checkpointing through `SessionStore.checkpoint`) | `packages/session/session-checkpoint-policy` |
| PTC runtime seam + Python backend (`PtcRuntime` Service Definition + `run` in a fresh CPython subprocess with top-level await/return, line-JSON binding bridge, wall-clock/abort/output caps, orthogonal failure taxonomy) | `packages/ptc-runtime/{ptc-runtime,ptc-runtime-node}` + `packages/experimental/ptc-runtime-python` |
| PTC mode `run_code` tool (language-aware schema flavor, nested tool sub-dispatch via the `tools` binding, `tool/ptc-dispatch*` event log, curated outer result) | `packages/core/tools/src/ptc.ts` |
...
| Context compaction (pre-step pressure + `CONTEXT_WINDOW_EXCEEDED` recovery, surface-replace checkpoint transaction, optional tool-result pruner stage; pressure/retention budgets scaled by context window minus reserved completion tokens and `headroomTokens`) | `compaction/compaction-basic` + `compaction-tool-result-pruner` |
| Background jobs (`job_output`/`job_list`/`job_kill` + completion notices, per-owner cap; one bounded output ring per job at absolute byte offsets — registry-pumped pull sources, consuming `read` + non-consuming `readAt`, scope-layered `events.subscribe`, `settled{cause,awaited}`; archive admission answers the `job` family; no `job/*` session events) | `packages/jobs` (jobs-local + tool-jobs) |
...
| Session telemetry reporting (`ctx.sessionTelemetry` seam + capture coordinator: live/on-demand, `session-telemetry/record` redaction waterfall, `agent/error` ops records, handoff cursor; OTel backend `FEEDBACK_ONLY`/`DISABLED` with OTLP/HTTP export) | `packages/session/session-telemetry` + `session-telemetry-otel` |
| Session projections (`ctx.sessionProjections` registry API v2: `ProjectionDefinition` state-driven units, eager drive over committed events, `is`-gated change feed, `stateOf`/`snapshot`/`cachedSnapshot`, checkpoint/restore/hydrate read ladder; telemetry registers sessionStats/tokenUsage units) | `packages/session/session-projection` |
...
| Workspaces (`ctx.workspaces` registry: realpath-canonical path identity, stable uuid, ordered session account with attach/insertBefore/detach, live directory status; registry-global archive set with `workspace/session-activity` admission + `workspace/session-stop` and `stopActivity`, with the `turn` family answered by the agent registry's installed archive admission; registry-global pin set with most-recent-first order; first-use `initializeDefault` + durable `defaultWorkspaceId`) | `packages/workspace/workspace` |
| User settings (`ctx.settings` provider: namespace registration, resolved value = schema defaults → composition base → user section, revision-guarded writes with `SETTINGS_CONFLICT`, secret redaction, file provider with watchdog reload) | `packages/settings/{settings,settings-file}` |
...
| Terminal service domain + real PTY backend + model tool family (`ctx.terminals` seam: spawn/kill/read/signal/start_send/list + `TerminalError` 8-code set; `miniharness/terminal_bash` real backend: config/env with UTF-8 preamble + `133;D` prompt marker discipline, a terminal emulator, poll-readiness five-way settle, `inspect_activity` with opt-in shell-activity evidence (private `bash --rcfile` / zsh `ZDOTDIR` status file, POSIX only), and a platform-split provider — `pty` on POSIX, pywinpty ConPTY on Windows; `miniharness/tool_terminal` six model tools `terminal_open`/`terminal_send`/`terminal_read`/`terminal_signal`/`terminal_close`/`terminal_list` with byte-for-byte render parity, `maxResultBytes` truncation, and background `terminal_send` via `jobs.start(kind='pty-send')`; `miniharness/terminal_controller` browser terminal domain — `ctx.terminalController` Remote namespace `terminal/*` (environment/shells/list/create/follow/write/resize/rename/close), pyte-backed bounded recovery screen, per-follower UTF-8 frame budget, owner-scoped teardown, and the `sandbox/mode` change fence) | `packages/terminal/{terminal,terminal-bash,tool-terminal}` + `packages/api/terminal-controller` + `packages/shell/pwsh-local` |
...
| Workspace control Remote (`ctx.workspaceController`, Remote namespace `workspace/*`: create/initializeDefault/rename/delete/insertBefore/insertSessionBefore/archiveSession/unarchiveSession/pinSession/unpinSession + `follow` baseline/upsert/remove/order/archived/pinned increments; registry order + archived-session set + pin set + `workspace/changed` notifications; active archive folds to `workspace/session-active`) | `packages/api/workspace-controller` |
| Workspace file preview Remote (`ctx.workspaceFiles`, Remote namespace `workspaceFiles/*`: bounded line `read`, native-bytes `readBytes` (`options.range` window / `options.baseFile` base dir, whole file under the `maxFileBytes` cap), `stat`, workspace-scoped `list` + target-scoped `changes` over `fs.watch` and `fs/observed`; `workspace-file/*` error set) | `packages/api/workspace-files` |
| Settings + credentials Remote (`ctx.settingsController`/`ctx.credentialsController`, namespaces `settings/*` redacted describe + update/replace/mutate + document/preset-directory open, and `credentials/*` describe/set/unset; `settings/conflict`/`settings/rejected`/`credential/rejected`) | `packages/api/settings-controller` |
| Request-context plugins (`miniharness/context`: `time_context` per-step clock readings + browser-zone policy, `tmux_context` tmux location, `file_reference`+`file_reference_local` `@file` mention grammar + fuzzy index (`ctx.fileReferences`), `session_reference` cross-session bounded snapshots + `sessionReferenceResolver/candidates`, `agent_instructions` AGENTS.md discovery/budget rendering/reconcile) | `packages/context/{time-context,tmux-context,file-reference,file-reference-local,session-reference,agent-instructions}` |
| Capability seams (sandbox / credentials / authorization / subagent; see details below) | capability seams docs |
| Continuable subagents (durable child sessions, async settlement, lifecycle events, archive admission, activation capacity, control tools, image prompt admission through the attachment store; see details below) | `packages/subagent` |
...
**Web transport layer + browser frontend.** Started with `--profile web`. Two-envelope RPC (`client-request` / `server-response`), a WebApi unary session service, and the Remote stream wire (`open` / `item` / `end` / `cancel` / `error` frames over a single `/api/remote.mux` WebSocket, exact-key frames, one bounded 256 KiB uplink inbox per stream whose overflow or post-`end` item fails just that stream with `gateway/uplink-overflow` / `gateway/protocol`). Each stream method reads its uplink through an invocation context that decodes every item against the method's codec (lossless-JSON validation when none is declared), a rejected item failing the stream with `gateway/input-invalid`. A unary result carrying bytes answers with `multipart/form-data` (`metadata` part plus one `bytes-<i>` part per attachment, the JSON slot left `null`), which the client rewrites into `Uint8Array` along each descriptor path.
 A `$events` registry forwards `api-session/*` and settles the `approval/request` waterfall through `$events/result`; `session/follow` and `session/control` carry the streams. Its wire client holds one handle per logical stream, sending uplink items behind the `open` frame and half-closing them. The approval bridge connects the async `tools/ask` gate to the `$events` waterfall. The FastAPI carrier mirrors the gateway `stream-server.ts` / `handler.ts` status-code chain. Session-log export at `GET /api/session.export` zips the root session, subagent descendants, and referenced media, keeping the 200 / 400 / 404 / 501 / 500 status chain with a private error shell. The productized `webui/` frontend is a standalone React project at the repo top that depends only on the wire contract: session list and create, Trajectory (virtualized windowing, Overview collapsed view, full-text search), approval waterfall, and queue/jobs panel; its `vite build` output is served from `MINIHARNESS_WEBUI_DIST`. The `web/static/` vanilla SPA is kept as a teaching reference only and does not work against the current backend.
...
- Shell: the `ctx.shell` executor family returns a `ShellExecution` handle (`execute(spec)`; `resolve` fills `workdir` / `timeoutMs` / `onExpiry: kill|none` / output budgets; non-consuming `observed` readers beside a consuming `readOutput()`; `kill()`; memoized `result()`), local and sandbox-confined providers; the `ctx.shellEnv` registry (`DSH_HOME` / `DSH_SHELL` / `DSH_SESSION_ID`, plus reserved `DSH_PROFILE` / `DSH_PROFILE_DIR` from the profile context); and the `tool_bash` model tool with `run_in_background` and `promoteOnTimeout` (a foreground timeout is promoted to a background job with the `promoted` result kind and `stopped` reason).
...
│   │   ├── deepseek.py      # DeepSeek Messages API adapter (httpx)
│   │   ├── deepseek_messages.py # Messages wire serialization + SSE translation
