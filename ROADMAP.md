...
下一主线：**web 浏览器半**（上游 `packages/bundle/web-app` + `packages/client`）——传输层（`--profile web`）已落地：四象限 RPC 信封、WebApi unary 会话服务、mux/host SSE 事件流、FastAPI 载体与启动器。后续在其上做浏览器 GUI（Trajectory、审批面板、命令/配置界面）时与后端同一事件循环即可，无需线程桥接。

后置：

- **web 浏览器半**：浏览器 GUI（上游 `packages/bundle/web-app` + `packages/client`）：Trajectory 折叠、审批面板、命令/配置界面；FastAPI + React 产品化与后端同一事件循环。
