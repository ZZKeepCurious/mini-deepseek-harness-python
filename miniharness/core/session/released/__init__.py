"""released 会话格式域：v0/v1/v2 物理编解码 + 相邻迁移链（v0→v1→v2→v3）。
...
对应 dsh 真实源码：
  ...
  * `packages/session/session-format-v2-to-v3/src/*`（system head 提升 + PTC 词汇
    改名 + canonical 信封，上游 dsh-v0.1.5-alpha.1）。
...
v3 校验（validate_v3.py：assertV3Event 信封 + 系统头/开步/压缩头保护 + 关系投影）
与 v2→v3 迁移接续（2026-09-09）。
"""
from .helpers import (
    SAFE_INT_MAX,
    SessionFormatError,
