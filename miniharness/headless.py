...
def run_headless(
    ...
    *,
    ...
    tools = tools or _default_tools(ctx)
...
def _default_tools(ctx: Context) -> ToolRegistry:
...
if __name__ == "__main__":
    # 直接运行：python -m miniharness.headless "task"
    task = " ".join(sys.argv[1:])
    if task.strip() == "":
        sys.stderr.write('error: a task is required, for example: python -m miniharness.headless "run the tests"\n')
        sys.exit(1)
    headless_main(task)
