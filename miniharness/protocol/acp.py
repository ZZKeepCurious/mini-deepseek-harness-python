...
def _route_label(route: dict, other: dict) -> str:
    """路由显示标签（上游 model-selection.ts routeLabel：provider 相同只显 model）。

    @param route 当前展示的选中路由。
    @param other 对照路由（判断 provider 是否相同）。
    """
    return route.get("model") if route.get("provider") == other.get("provider") \
        else f"{route.get('provider')}/{route.get('model')}"


_CONTEXT_SUMMARY_MAX_CHARS = 120


def _bound_context_summary(summary: str) -> str:
    """截断 notice summary 到 120 字符 + 省略号（上游 message.ts CONTEXT_SUMMARY_MAX_CHARS）。"""
    return summary if len(summary) <= _CONTEXT_SUMMARY_MAX_CHARS \
        else f"{summary[:_CONTEXT_SUMMARY_MAX_CHARS - 1]}…"
...
class AcpServer:
    ...
    def _install_model_selection(self, ctx: Context, record: dict) -> None:
        """每会话一次安装模型选择施加监听（resume 复用同一 ctx）。

        对齐上游 model-selection.ts installModelSelection 的三段：
        `system-prompt/assemble`（装配期快照到 assembled，注入 variables）、
        `agent/request`（选择施加）、`agent/pre-step`（provider/model 变化时
        追加 durable modelSwitchNotice user 消息）。mini 无 assembly variables
        面（system-prompt 服务不消费 provider/model variables），故第一段不
        承载（登记载体差异）；agent/request 与 agent/pre-step 两段实现。
        """
        ...
        ctx.on("agent/pre-step",
               self._make_selection_notice_listener(record))
        ctx._miniharness_acp_model_bound = True

    def _make_selection_notice_listener(self, record: dict):
        async def listener(payload: dict, next_fn: Callable):
            return await self._apply_selection_notice(record, payload, next_fn)
        return listener
    ...
    async def _apply_selection_notice(self, record: dict, payload: dict,
                                      next_fn: Callable) -> dict:
        """agent/pre-step 尾段：provider/model 变化时追加 modelSwitchNotice。

        对齐上游 model-selection.ts:113-126：reject/已中止/空 step 不加；
        selected（assembled）与已持久化 requestHeader config 同路由时不加；
        相异则把 notice 追加到决策 messages 末尾。mini 同步单飞回合内
        `current` 即装配快照（pin_turn），与上游 `assembled` 语义等价。
        """
        decision = await next_fn()
        if not isinstance(decision, dict) or decision.get("kind") == "reject":
            return decision
        ...
        if not isinstance(messages, list) or len(messages) == 0 \
                and (step == 1 or (isinstance(original, list) and len(original) > 0)):
            return decision
        selected = record["model_control"].current
        session = record.get("session")
        previous = session.request_header() if session is not None else None
        previous_config = previous.get("config") if isinstance(previous, Mapping) else None
        if selected is None or not isinstance(previous_config, Mapping):
            return decision
        if previous_config.get("provider") == selected.get("provider") \
                and previous_config.get("model") == selected.get("model"):
            return decision
        previous_label = _route_label(previous_config, selected)
        selected_label = _route_label(selected, previous_config)
        notice = create_message("user", [
            text_block(
                "[model changed: assistant turns above this point were generated by "
                f"{previous_label}; the session continues with {selected_label}]"),
        ], {
            "kind": "model-selection",
            "form": "notice",
            "summary": _bound_context_summary(f"{previous_label} → {selected_label}"),
        })
