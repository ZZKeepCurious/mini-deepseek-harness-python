...
**web 传输层与浏览器前端**：`--profile web` 启动。两信封 RPC（`client-request` / `server-response`）、WebApi unary 会话服务、Remote 流 wire（单条 `/api/remote.mux` WebSocket 承载 open / item / end / cancel / error 帧，帧字段集合精确匹配；每条流一条 256 KiB 有界上行 inbox，超限或 `end` 后再来 `item` 以 `gateway/uplink-overflow` / `gateway/protocol` 只中止该流）；每个流方法经调用上下文读自己的上行，逐项过该方法声明的 codec（未声明则走无损 JSON 校验），被拒项以 `gateway/input-invalid` 中止该流；unary 结果含字节时改用 `multipart/form-data`（`metadata` part + 每附件一个 `bytes-<i>` part，JSON 槽位留 `null`），客户端按描述符 path 写回 `Uint8Array`；`$events` 注册表（转发 api-session/*，`approval/request` waterfall 经 `$events/result` 结算）、`session/follow` / `session/control` 流；客户端为每条逻辑流持一个句柄，在 `open` 帧之后发上行项并可半关；审批桥把 async `tools/ask` 接到 `$events` waterfall；FastAPI 载体与 gateway 的 `stream-server.ts` / `handler.ts` 状态码链一致；会话日志导出 `GET /api/session.export`，把 root、子代理后代和被引用媒体打包成 zip，沿用 200 / 400 / 404 / 501 / 500 状态码链，错误走私有信封外壳；产品化前端 `webui/` 是仓库顶层的独立 React 工程，只依赖 wire 约定，提供会话列表与新建、Trajectory（虚拟化窗口、Overview 折叠视图、全文搜索）、审批瀑布、队列与作业面板，`vite build` 产物经 `MINIHARNESS_WEBUI_DIST` 由后端静态承载；`web/static/` 的 vanilla SPA 只作教学参照，消费旧 SSE wire，对新后端不工作。
...
│   │   │   ├── persistence.py
│   │   │   ├── generation.py  # 多代读侧 + migrate-on-open
│   │   │   ├── released/      # released v0/v1 codec + v0→v1→v2→v3 相邻迁移链
│   │   │   └── zstd_frames.py
...
│   │   └── deepseek_files/  # DeepSeek Files API 执行簇（图片输入请求）
