...
| 持久化（JSONL / SQLite、默认 zstd 拼接帧容器一行一事件、`root/--<projectKey>--/<encoded-id>/session.v2.jsonl[.zstd]` 布局、header + `SESSION_FORMAT_VERSION=2` 双向拒读、编码/布局错配响亮拒绝、flush 栅栏、崩溃恢复、多代读侧经 v0→v1→v2 迁移链就地迁移 released v0/v1 旧产物） | `packages/session/session-persistence` + `session-format-*` |
...
│   │   │   ├── released/      # released v0/v1 codec + v0→v1→v2 相邻迁移链
│   │   │   └── zstd_frames.py
