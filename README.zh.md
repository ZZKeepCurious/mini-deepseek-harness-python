...
| 持久化（JSONL / SQLite、默认 zstd 拼接帧容器一行一事件、`root/--<projectKey>--/<encoded-id>/session.jsonl[.zstd]` 布局、header + `SESSION_FORMAT_VERSION=2` 双向拒读、编码/布局错配响亮拒绝、flush 栅栏、崩溃恢复） | `packages/session/session-persistence` |
