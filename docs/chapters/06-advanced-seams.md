...
**沙箱发布/部署平台矩阵与 fail-closed 契约（release/ops 视角）**：上面按平台选链只讲了"怎么选"，这里把它收敛成操作用（release/ops）视角的矩阵：
...
**凭据四层**（`credentials_local.py`，对应上游 `credentials-local`）：`env > file > project-env > user-env` 按信任度排序——继承环境只读胜出（CI secret / `-e` 是显式意图且进程内不可编辑）、管理文件层可写（`set`/`unset` 读-改-写补丁单键，外部编辑合并、删掉的条目不残留）、project `.env` 优先于 user `.env`。文档解析严格：非映射根 / 非 POSIX 标识符 key / 非字符串 / 空串值整体拒绝，绝不静默跳过；`describe` 报告 `{configured, source, writable}`（只有 env 层不可写）；env 已提供时 `set`/`unset` 拒绝（写了也被遮蔽成无效果）；POSIX 上组/其他可读的文档读前直接拒绝（Windows 无 mode 可查则跳过）。载体简化/决策：文档用 JSON 替代 YAML（解析语义不变；JSON 重复键 fail-closed 更稳，为受评审设计决策）；跨进程写锁已用 `filelock` 闭合（`withFileLock` 同款，`credentials_local.py:553`，写入等待 30s——`DOCUMENT_LOCK_WAIT_SECONDS` 覆盖 atomic-write 2s 默认）。凭据变更感知双通道：**① 读侧热重载（缺省 pull 兜底）**——`resolve`/`describe`/`read_record` 等读入口先 `_refresh_if_changed()`——`os.stat` 比对 mtime/size，变了才重解析整表（外部编辑/删除即时生效），写路径 `_reconcile_from_disk` 折叠不变；无撕裂（atomic write os.replace），读侧不需锁。**② 可选文件 watch（P2-2，显式开启）**——`LocalCredentialProvider(watch=True)`（或 `CredentialsService` 装配）以 watchdog 主动监听外部编辑（对齐上游 chokidar watch + reconcileFromDisk 文本比对 + 逐 entry fan-out），缺省关闭（与 ACP 持久化同构，避免默认起线程）；watch 事件去抖单飞折叠，逐 entry 发 `credentials/reference-updated`/`credentials/record-updated`（经 `CredentialsService` 转发 ctx 事件），自写 echo 由文本比对抑制。`credentials_local.py` 还实现记录（record）服务侧五件套：`read_record`/`describe_record`/`list_records`/`modify_record`/`delete_record`（modify_record 是唯一写路径：持锁内 reconcile 折叠外部编辑 → mutate 收到当前记录返回替换/拒绝 → 写前准入与读路径同一套 → 原子落盘；`<scope>/<id>` 键语法 `[a-z][a-z0-9-]*`，段非法逐字 TypeError）。
