...
!!! warning "早期简化形态"
    ...
    - **`Session.append` 签名**：本章为 `append(event: dict)`；实现为 `append(type_, data=None, surfaceOp=None, sourceEventSeqs=None)`（`core/session/session.py:69`），信封 `{type, seq, time, data}` 由 `seq == len(log)` 自动编号。
    - **事件词汇表**：本章只讲 8 个核心类型；实现 `KNOWN_TYPES` 共 **30 个**（`core/session/types.py`），另含 `request/header`、`request/context`、`session/end-seed`、`agent/inbox/spliced`、`approval/asked|decided|policy`、`hook/invoked|result`、`llm/retry|retry-started`、`compaction/start|summary|end|prune`、`plan/mode`、`command/run|done`、`goal/change`、`subagent/descriptor`、`sandbox/mode`（上游全集 44 类，读真实上游日志遇超集类型时 fail-closed 拒读）。
