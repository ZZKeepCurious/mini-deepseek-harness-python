...
!!! warning "早期简化形态"
    ...
    - **事件词汇表**：本章只讲 8 个核心类型；实现 `KNOWN_TYPES` 共 **28 个**（`core/session/types.py`），另含 `request/header`、`request/context`、`session/end-seed`、`approval/asked|decided|policy`、`hook/invoked|result`、`llm/retry|retry-started`、`compaction/start|summary|end`、`plan/mode`、`command/run|done`、`goal/change`、`agent/inbox/spliced`（读真实上游日志遇超集类型时 fail-closed 拒读）。
