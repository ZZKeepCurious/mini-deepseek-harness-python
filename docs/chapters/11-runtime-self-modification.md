...
自我修改在上游不是"agent 任意改配置"：`tool-cordis` 的说明文档第一句就写了（已核实，`tool-cordis/README.md:19`）：
...
2. **运行中不可重复 run / 不可 undefine**：`run` 抛 RuntimeError；`undefine` 要求先 `stop`。
...
> 第 08-11 章至此构成"组合层"闭环：预设（静态装配）→ 干预（运行时控制）→ 轨迹（事后投影）→ 动态插件（运行中自修改），审批（09 章 §9.5）管住干预通道。外部入口（07 章 §7.6-7.8）已复现；下一步进入第 12 章：异步化与并行工具执行（ROADMAP 阶段 7，与 dsh 的最大差距）。