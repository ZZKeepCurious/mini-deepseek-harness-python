...
| `loader/`（model/utils/patch/entry/group/tree/loader/include） | `vendor/loader/src/{index,internal,config/{entry,group,isolate,tree,utils}}.ts` + `vendor/include/src/index.ts` | cordis 组件系统活树：EntryTree 扁平 store + `entries()/resolve()/import_()/write()`；Entry 生命周期（update/disabled/init、`loader/partial-dispose`、`internal/plugin` 归属）+ module 旧方言 apply 桥；EntryGroup 一级子列表宿主（create/remove/update/stop）+ Group 插件；Loader 服务（`internal/config` 树载体字面/普通条目 interpolate、`internal/update` 写回 + 重载日志、`internal/plugin` 自销毁回写 disabled）+ `apply_entry_patches` + Include（文件读写/initial/dump `!!js` 原样、rename 重试）。**方言**：条目 YAML 用上游 `entryListSchema`（JSON_SCHEMA + `!!js`）。载体/适配（登记 §3）：同步门面 + `ctx.plugin(parent=)` 显式归属、baseUrl 沿 ctx 父链上溯、`settle_gathered` 瞬态 loop 结算。**未移植/不适用**：`config/isolate.ts`（intercept/isolate 选项）、`Loader.Intercept.await`、`internal.ts`（Node ESM 内部加载器，架构不适用）——详见 verified-diffs §3.26 |
