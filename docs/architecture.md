...
| `web/launcher.py` | `packages/host/webserver`（Config：host 两值 + port 0）+ `api/gateway` heartbeat | host/port 读 MINIHARNESS_WEB_HOST/PORT（上游组合配置节，简化标注）；**心跳对齐上游 gateway 契约**：`uvicorn_options()` 设 `ws_ping_interval=2 / ws_ping_timeout=4`（transport 级 Ping 2s + 连续 2 周期无 Pong 判死 ≈ 上游 `websocketHeartbeatIntervalMs` @default 2000 + `MAX_MISSED_HEARTBEATS=2` terminate） |
