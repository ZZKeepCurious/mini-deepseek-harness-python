...
| JSONL 持久化 + 崩溃恢复 | **zstd 拼接帧容器** + 一行一事件 + generation 文件名 `session.v3.jsonl[.zstd]`（`compression='none'` 退回明文 `.jsonl`）；启动回放时合成 `interrupted` turn（`commit_repair` 追加 closers），torn 尾部读路径截断；released 旧格式经相邻迁移链（v0→v1→v2→v3）读入 | seq 连续；未知事件类型拒绝加载（fail-closed，`ignorable: true` 豁免放行）；`SESSION_FORMAT_VERSION = 2` 双向拒读 |
